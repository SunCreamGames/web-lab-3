...
public class AccordionsController : ControllerBase
{
    ...
    {
        var res = new List<AccordionResponse>();
        foreach (var accordion in _dbContext.Accordions)
        {
            res.Add(new AccordionResponse()
            {
                Id = accordion.Id,
    ...
    public async Task<IActionResult> Add([FromBody] CreateAccordionRequest accordion)
    {
        ...
        {
            return BadRequest($"An error occured while creating an accordion. {ex.Message}");
        }
    }
    ...
    public async Task<IActionResult> Delete([FromRoute] string accordionId)
    {
        try
        {
